...
O esforço central dos pesquisadores, então, consiste em encontrar formas de reduzir o tamanho da representação do espaço de estados, construindo representações simbólicas ou abstratas \cite{clarkeIntroductionModelChecking2018}.
	...
	Como visto no Capítulo~\ref{ch:code}, a representação de número é diretamente em BDDs, i.e., como um vetor de bits.
	Sobre o \textit{overhead} geral do tempo de compilação do \texttt{llull-BDD}, encontra-se justificado  provavelmente pela representação intermediária entre a AST e o BDD.
	Por fim, a heurística que melhor desempenhou, no geral, foi a \textit{sifting}, seguido da FORCE, com algumas exceções notáveis, ou seja, a sensibilidade para uma heurística de ordenamento depende a instância.
