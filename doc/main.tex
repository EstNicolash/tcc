...
Por outro lado, a heurística \textit{sifting} se revelou a melhor, levando os dois verificadores a resolverem mais instâncias. 
...
\FloatBarrier
	...
	\begin{itemize}
		...
		\item \textbf{\textit{Synapse}:} apenas o verificador NuSMV conseguiu terminar pelo menos uma instância dadas as restrições com todas as heurísticas. Com elas, foi possível alcançar até $N=6$ para \textit{sifting} e $N=4$ para FORCE (em comparação, a heurística padrão apenas resolve $N=3$). Já \texttt{llull-bdd} conseguiu resolver com apenas com a heurística FORCE. Como não há dados suficientes para uma boa visualização é para realizar os testes estatísticas, optou-se por omitir na analise deste capítulo.
		\item \textbf{\textit{Train Gate}:} Mais um cenário em que a ordem de declaração padrão (\textit{default}) já se encontra em um limiar ótimo, assim como o caso da instância Sokoban. Como não há novidades para análise, optou-se por não apresentar neste capítulo.
	\end{itemize}
	...
	Um caso, por exemplo, é o uso de \textit{Algebraic Decision Diagrams} (ADDs ou MTBDDs), que reduzem o número de nós para representar números, melhorando a eficiência de operações matemáticas. Como visto no Capítulo~\autoref{ch:code}, a representação de número é diretamente em BDDs, i.e., como um vetor de bits.
	Por fim, as heurísticas que melhor desempenharam, no geral, foi a \textit{sifting}, seguindo da FORCE, com algumas exceções notáveis.
