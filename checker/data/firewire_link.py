...
def generate_instance(N: int) -> str:
    ...
    # Mapping variables: Original floor [0..N] -> Translated [1..N+1]
    # Boundaries: -1 -> 0, N -> N+1
    lines = [
        f"-- elevator2_{N}.ssmv",
    ...
    lines.extend(["  next(t) := case"])
    # Condition: Original t > -1 becomes t > 0
    lines.append(f"    action = act_ctrl & ctrl = st_ctrl_wait & v = 0 & ldir = 0 & t > 0 : t - 1;")
    lines.append(f"    action = act_ctrl & ctrl = st_ctrl_wait & v = 0 & ldir = 1 & t < {N+1} : t + 1;")
    for i in range(N):
    ...
    lines.extend(["    TRUE : t;", "  esac;", ""])

    # Control 'v', 'ldir', 'cabin', 'ctrl' and 'req' transitions
    # (These blocks follow the flattened structure, ensuring case expressions are at the top level)
    lines.extend([
        "  next(v) := case action = act_ctrl & ctrl = st_ctrl_done : 1; action = act_cabin & cabin = st_cabin_open : 0; TRUE : v; esac;",
        "",
        f"  next(ldir) := case action = act_ctrl & ctrl = st_ctrl_work & (t = 0 | t = {N+1}) : 1 - ldir; TRUE : ldir; esac;",
        "",
        "  next(cabin) := case",
    ...
    lines.extend([
        "",
        "-- ── Safety: Doors only open at target floor ──",
        "CTLSPEC AG (cabin = st_cabin_open -> t = p);",
        "",
        "-- ── Reachability: The highest floor can be served ──",
        f"CTLSPEC EF (p = {N+1} & cabin = st_cabin_open);"
    ])
